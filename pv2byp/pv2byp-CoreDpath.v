...
(
  ...
  // Bypass Signals
  input wire [2:0] rs_bypass,
  input wire [2:0] rt_bypass,
  ...
  // Jump reg address (use forwarded rs for JR/JALR so producer in X/M/W is correct)
  ...
  // Bypass mux for rs (op0): regfile or forwarded value from X, M, or W
  wire [31:0] rs_bypass_out_Dhl
    = ( rs_bypass[2] ) ? execute_mux_out_Xhl
    : ( rs_bypass[1] ) ? wb_mux_out_Mhl
    : ( rs_bypass[0] ) ? wb_mux_out_Whl
    :                     rf_rdata0_Dhl;

  assign jumpreg_targ_Dhl = rs_bypass_out_Dhl;

  // Bypass mux for rt (op1)
  wire [31:0] rt_bypass_out_Dhl
    = ( rt_bypass[2] ) ? execute_mux_out_Xhl
    : ( rt_bypass[1] ) ? wb_mux_out_Mhl
    : ( rt_bypass[0] ) ? wb_mux_out_Whl
    :                     rf_rdata1_Dhl;

  // Operand 0 mux

  wire [31:0] op0_mux_out_Dhl
    = ( op0_mux_sel_Dhl == 2'd0 ) ? rs_bypass_out_Dhl 
  ...
  wire [31:0] op1_mux_out_Dhl
    = ( op1_mux_sel_Dhl == 3'd0 ) ? rt_bypass_out_Dhl
  ...
  // wdata with bypassing (store data = rt, possibly bypassed)
  wire [31:0] wdata_Dhl = rt_bypass_out_Dhl;
